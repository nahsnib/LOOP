...
def ability_killer(actor, all_chars):
    targets = get_targets_in_same_region(actor, all_chars)
    if targets:
        victim = min(targets, key=lambda x: x.sanity)
        print(f"   🔪 [殺手] {actor.name} 在陰影中現身，殺害了 {victim.name}！")
        victim.is_dead = True
        victim.sanity = 0

def ability_spreader(actor, all_chars):
    targets = get_targets_in_same_region(actor, all_chars)
    if targets:
        print(f"   👿 [散播者] {actor.name} 散布了謠言...")
        for t in targets:
            print(f"      -> {t.name} 感到恐慌 (精神-1)")
            t.sanity -= 1
            check_sanity_status(t)

def ability_cultist(actor, all_chars):
    targets = get_targets_in_same_region(actor, all_chars)
    if targets:
        print(f"   🙏 [邪教徒] {actor.name} 正在進行詭異的儀式...")
        for t in targets:
            t.sanity -= 1
            check_sanity_status(t)
